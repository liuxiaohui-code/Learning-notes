...
1. Node.js 是一个跨平台、后端、开源的 JavaScript 运行时环境
2. 构建在 Chrome 的 V8 JavaScript 引擎之上，并在 Web 浏览器之外执行 JS 代码
...
1. 卸载本地 node
2. 下载地址: linux`curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.7/install.sh | bash`,windows`choco install nvm`
3. 配置镜像源,nvm 安装目录,找到 setting.txt 文件加上如下两行
...
> 1. 安装目录 英文且不允许空格
> 2. exit status 1:乱码，或出现 exit status 1: You do not have sufficient privilege to perform this operation.; 用管理员身份打开 cmd 命令行工具，再执行`nvm use xxxx`即可。
...
nvm npm_mirror [url]        # : Set the npm mirror. Defaults to https://github.com/npm/cli/archive/. Leave [url] blank to 	defaulturl.
